...
class Teacher:
    ...
    def predict1(self,data):
        teacher = self.model()
        checkpoint = torch.load('/home/liubochao/Downloads/MEAL-master/checkpoint/vgg19_BN/ckpt.t7')
        ...
        output = output.max(dim=1)[1]
        out.append(output)
        model_predictions["model_0"] = out
        model_predictions["model_1"] = out
        model_predictions["model_2"] = out
        model_predictions["model_3"] = out
        model_predictions["model_4"] = out
        counts, model_counts = self.aggregate(model_predictions, len(data))
        counts = counts.apply_(self.addnoise)
